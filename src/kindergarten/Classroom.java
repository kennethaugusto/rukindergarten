...
public class Classroom {
    ...
    public void seatStudents () {
        ...
        // go through the array one seat at a time
        for (int i = 0; i < seatingLocation.length; i++) {
            for (int j = 0; j < seatingLocation[i].length; j++) {

                // read which spots are open to sit at : note - true represents a seat is open in the column
                if (seatingLocation[i][j] == true && studentsSitting[i][j] == null) {
                    // start from the front of the studentsInLine singly linked list and put it in the open seat
                    studentsSitting[i][j] = studentsInLine.getStudent();
                    // when it's the end of the studentsInLine, will null and return to avoid errors
                    if (studentsInLine.getNext() == null) {
                        studentsInLine = null;
                        return;
                    
                    }
                    // removes one student at a time because still in for loop
    ...
    public void insertMusicalChairs () {
        // creates new ArrayList 'newLine' and puts studentsSitting to newLine, not pointing yet
        ArrayList<Student> newLine = new ArrayList<>();
        for (int i = 0; i < studentsSitting.length; i++) {
            for (int j = 0; j < studentsSitting[i].length; j++) {
                if (studentsSitting[i][j] != null) {
                    newLine.add(studentsSitting[i][j]);
                }
                studentsSitting[i][j] = null;
            }
        }
    
        // add/remove first student to the end of the list
        newLine.add(0, newLine.get(newLine.size()-1));
        newLine.remove(newLine.size()-1);
    
        // create first node and set musicalChairs to it
        SNode front = new SNode(newLine.get(0), null);
        musicalChairs = front;
    
        // add remaining students to the list
        for (int i = 1; i < newLine.size(); i++) {
            SNode newNode = new SNode(newLine.get(i), null);
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        // goes to the randomIndex
        while (randomStudent > 0) {
            ptr = ptr.getNext();
            randomStudent--;
        }
        
        // case: if musicalChairs is size 1, insert student into line by height & clears musicalChairs
        if (musicalChairs == ptr.getNext()) {
        ...
        // remove node to right of ptr and insert student into line by height
        SNode removeStudent = ptr.getNext();
        ptr.setNext(removeStudent.getNext());
        insertByHeight(removeStudent.getStudent());
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        // case: no students in line
        if (studentsInLine == null) {
            studentsInLine = insertStudent;
            return;
        }
        // case: students in line
        SNode ptr;
        SNode prev = null;
        for (ptr = studentsInLine; ptr != null; ptr = ptr.getNext()) {
            // check if height of current student greater than height of student to insert
            if (ptr.getStudent().getHeight() > studentToInsert.getHeight()) {
                // if student is shorter, insert at beginning
                if (prev == null) {
                    insertStudent.setNext(studentsInLine);
                    studentsInLine = insertStudent;
                    return;
                }
                // if not, insert new student in between previous node and current node
                else {
                    prev.setNext(insertStudent);
                    insertStudent.setNext(ptr);
                    return;
                }
            }
            // if current student not taller than student to insert, move to next node
            prev = ptr;
        }
        // if end of list is reached without inserting anything, insert new student at end
        prev.setNext(insertStudent);
    }    
    ...
    public void eliminateLosingStudents() {
        ...
        // find number of students currently on musical chairs
        int size = 0;
        SNode ptr;
        ...
        size++;
        // while more than 1 student, call moveRandomStudentFromChairsToLine
        // (fixed to for loop?)
        // remove losing students until only one student remains
        for (int i = size; i > 1; i--) {
            // move losing student from chairs to line
    ...
    public void seatMusicalChairsWinner () {
        ...
        // case: musicalChairs empty
        if (musicalChairs == null) {
            return;
        }
        // check if more than student in musicalChairs list
